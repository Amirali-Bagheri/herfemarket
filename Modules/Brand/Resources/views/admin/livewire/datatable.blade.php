<div>
    ...
    <div class="intro-y col-span-12 flex flex-wrap sm:flex-no-wrap items-center mt-2 ltr">
        <a href="{{ route('admin.brands.create') }}" class="btn btn-primary shadow-md mr-2">برند
            جدید
        </a>
        ...
        <div class="hidden md:block mx-auto text-gray-600">
            نمایش {{ $brands->firstItem() }} تا {{ $brands->lastItem() }} از {{ $brands->total() }} مورد
        </div>
            ...
            <tr>
                <th class="text-center whitespace-no-wrap">
                    <input type="checkbox" class="form-check-input mr-2"
                           @if(count($selected)===$brands->total()) checked
...
تعداد محصولات
            ...
            @forelse($brands as $brand)
                ...
                <tr class="intro-x">
                    ...
                    <td class="text-center w-5">{{$brand->id}}</td>
                    ...
                    <td class="text-center">{{ $brand->created_at_human_ago}}</td>


                    {{-- <td class="text-center">{{ verta($brand->last_login_at)->formatDifference()  }}</td> --}}
                    <td class="table-report__action w-40 text-center">
                        <a href="{{route('admin.brands.show',$brand->id)}}" style="margin: 5px;">
                            <i class="fa-regular fa-eye"></i>
                        </a>
                        <a style="margin: 5px;" href="{{route('admin.brands.update',$brand->id)}}">
                        ...
                        <a style="margin: 5px;"
                           ...
                           wire:click="destroy({{$brand->id}})"
                            {{--                           href="{{route('admin.brands.delete',$brand->id)}}"--}}
                        >
    ...
    <div class="intro-y col-span-12 flex flex-wrap sm:flex-row sm:flex-no-wrap">
        ...
        <div class="form-inline ml-auto w-full">
            {{ $brands->links('admin.layouts.livewire_pagination') }}
        </div>
