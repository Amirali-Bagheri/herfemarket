...
class nusoap_base
{
            ...
            case is_object($val):
                ...
                if (get_class($val) == 'soapval') {
                    ...
                    if (! $name) {
                        ...
                        $this->debug("In serialize_val, do not override name $name for element name for class " .
                            get_class($val));
                    }
                    foreach (get_object_vars($val) as $k => $v) {
                        $pXml = isset($pXml) ? $pXml .
                            $this->serialize_val($v, $k, false, false, false, false, $use) : $this->serialize_val($v, $k, false, false, false, false, $use);
                    }
                }
                if (isset($type) && isset($type_prefix)) {
                    $type_str = " xsi:type=\"$type_prefix:$type\"";
    ...
    public function serializeEnvelope($body, $headers = false, $namespaces = [], $style = 'rpc', $use = 'encoded', $encodingStyle = 'http://schemas.xmlsoap.org/soap/encoding/')
    {
        ...
        $this->debug("In serializeEnvelope length=" . strlen($body) . " body (max 1000 characters)=" . substr($body, 0, 1000) .
            ...
            if (isset($attrs['elements']) && (count($attrs['elements']) > 0)) {
                foreach ($attrs['elements'] as $element => $eParts) {
                    if (isset($eParts['ref'])) {
                        ...
                        $contentStr .= "   <$schemaPrefix:element name=\"$element\" type=\"" .
...
class soap_transport_http extends nusoap_base
{
     ...
     * @param array $certRequest (keys must be cainfofile (optional), sslcertfile, sslkeyfile, passphrase, certpassword
     *     (optional), verifypeer (optional), verifyhost (optional): see corresponding options in cURL docs)
     * @access   public
     */
        ...
        if ($authtype == 'basic') {
            ...
            if (isset($digestRequest['nonce'])) {
                ...
                if ($digestRequest['qop'] != '') {
                    $unhashedDigest = $HA1 . ':' . $nonce . ':' . sprintf("%08d", $digestRequest['nc']) . ':' . $cnonce . ':' .
                ...
                $this->setHeader('Authorization', 'Digest username="' . $username . '", realm="' . $digestRequest['realm'] .
                    '", nonce="' . $nonce . '", uri="' . $this->digest_uri . $opaque . '", cnonce="' . $cnonce . '", nc=' .
                    sprintf("%08x", $digestRequest['nc']) . ', qop="' . $digestRequest['qop'] . '", response="' . $hashedDigest .
                    '"');
            }
...
class nusoap_server extends nusoap_base
{
    ...
    {
        $this->debug('in invoke_method, methodname=' . $this->methodname . ' methodURI=' . $this->methodURI . ' SOAPAction=' .
            $this->SOAPAction);
        ...
        if ($this->wsdl) {
            if ($this->opData = $this->wsdl->getOperationData($this->methodname)) {
                ...
                $this->debug('in invoke_method, found WSDL soapAction=' . $this->SOAPAction . ' for operation=' .
                    $this->opData['name']);
                $this->appendDebug('opData=' . $this->varDump($this->opData));
                $this->methodname = $this->opData['name'];
            } else {
                $this->debug('in invoke_method, no WSDL for operation=' . $this->methodname);
                $this->fault('SOAP-ENV:Client', "Operation '" . $this->methodname .
        ...
        if ($this->wsdl) {
            ...
            if ($this->opData['style'] == 'rpc') {
                ...
                if ($this->opData['output']['use'] == 'literal') {
                    ...
                    if ($this->methodURI) {
                        $payload = '<ns1:' . $this->methodname . 'Response xmlns:ns1="' . $this->methodURI . '">' . $return_val .
     ...
     * @param mixed $style optional (rpc|document) or false Note: when 'document' is specified, parameter and return wrappers are
...
class wsdl extends nusoap_base
{
        ...
        if ($this->status == 'schema') {
            ...
            switch ($this->status) {
                case 'message':
                    if ($name == 'part') {
                        if (isset($attrs['type'])) {
                            $this->debug("msg " . $this->currentMessage . ": found part (with type) $attrs[name]: " .
                                implode(',', $attrs));
                            $this->messages[$this->currentMessage][$attrs['name']] = $attrs['type'];
                        }
                        if (isset($attrs['element'])) {
                            $this->debug("msg " . $this->currentMessage . ": found part (with element) $attrs[name]: " .
                                implode(',', $attrs));
    ...
    {
        $this->debug("in serializeType: name=$name, type=$type, use=$use, encodingStyle=$encodingStyle, unqualified=" .
        ...
        $phpType = $typeDef['phpType'];
        $this->debug("in serializeType: uqType: $uqType, ns: $ns, phptype: $phpType, arrayType: " .
        ...
        if ($phpType == 'struct') {
            ...
            if (is_array($value) && sizeof($value) >= 1) {
                ...
                $contents = '';
                foreach ($value as $k => $v) {
                    $this->debug("serializing array element: $k, " . (is_array($v) ? "array" : $v) .
            ...
            foreach ($typeDef['elements'] as $eName => $attrs) {
                ...
                if (isset($optionals)
                    ...
                    if (isset($attrs['minOccurs']) && $attrs['minOccurs'] <> '0') {
                        $this->debug("apparent error: no value provided for element $eName with minOccurs=" .
                            $attrs['minOccurs']);
                    }
                    // do nothing
                    ...
                    if (isset($attrs['maxOccurs']) && ($attrs['maxOccurs'] == 'unbounded' || $attrs['maxOccurs'] > 1) &&
                        isset($v) && is_array($v) && $this->isArraySimpleOrStruct($v) == 'arraySimple') {
                        $vv = $v;
                        foreach ($vv as $k => $v) {
     ...
     * @param string $style (rpc|document) optional The style for the operation Note: when 'document' is specified, parameter and
     *     return wrappers are created for you automatically
    ...
    public function __construct($xml, $encoding = 'UTF-8', $method = '', $decode_utf8 = true)
    {
        ...
        if (! empty($xml)) {
            ...
            if (! xml_parse($this->parser, $xml, true)) {
                ...
                $this->debug('parsed successfully, found root struct: ' . $this->root_struct . ' of name ' .
...
class nusoap_client extends nusoap_base
{
     ...
     * @return    mixed    response from SOAP call, normally an associative array mirroring the structure of the XML response,
     *     false for certain fatal errors
     * @access   public
     */
    ...
    public function checkWSDL()
    {
        ...
        if ($errstr = $this->wsdl->getError()) {
            ...
            $this->debug('got ' . count($this->operations) . ' operations from wsdl ' . $this->wsdlFile . ' for binding type ' .
                $this->bindingType);
        ...
        switch (true) {
            ...
            case preg_match('/^http/', $this->endpoint):
                ...
                if ($err = $http->getError()) {
                    ...
                    $this->debug('got response, length=' . strlen($this->responseData) . ' type=' .
