...
{{--
        <a href="{{ route('admin.notifications.create') }}" class="btn btn-primary shadow-md mr-2">اعلان جدید</a>
--}}
        <div class="dropdown">
            ...
            <div class="dropdown-menu w-40">
                <div class="dropdown-menu__content box p-2">
                    ...
                    @if (count($selected) > 0) <a href="#" data-toggle="modal"
        ...
        <div class="hidden md:block mx-auto text-gray-600">
            نمایش {{ $notifications->firstItem() }} تا {{ $notifications->lastItem() }} از {{ $notifications->total() }} مورد
        </div>
                ...
                <tr>
                    <th class="text-center whitespace-no-wrap">
                        <input type="checkbox" class="form-check-input mr-2"
                               @if(count($selected)===$notifications->total()) checked @endif wire:click.lazy="toggleSelectAll">
                    ...
                    <th class="text-center whitespace-no-wrap">
                        <a wire:click.prevent="sortBy('notifiable_id')" role="button" href="#">
                            نام و نام خانوادگی
                            @include('admin.layouts.livewire_sort_icon', ['field' => 'notifiable_id'])
                        </a>
                    </th>
                    ...
                    <th class="text-center whitespace-no-wrap">
                        <a wire:click.prevent="sortBy('type')" role="button" href="#">
                            نوع
                            @include('admin.layouts.livewire_sort_icon', ['field' => 'type'])
                        </a>
                    </th>

                    <th class="text-center whitespace-no-wrap">
                        <a wire:click.prevent="sortBy('read_at')" role="button" href="#">
                            دیده شده در
                            @include('admin.layouts.livewire_sort_icon', ['field' => 'read_at'])
                        </a>
                    </th>
            ...
            <tbody>
                @forelse ($notifications as $notification)
                    ...
                    <tr class="intro-x">
                        ...
                        <td class="text-center w-5">{{$notification->id}}</td>
                        <td class="text-center">{{$notification->notifiable->name}}</td>
                        <td class="text-center">{{$notification->type}}</td>
                        <td class="text-center">{{ $notification->read_at ? verta($notification->read_at)->formatDifference()  : '-' }}</td>
                        <td class="text-center">{{ $notification->created_at_human}}</td>

                        <td class="table-report__action w-50 text-center">
                            <a style="margin: 5px;"
                               href="#"
                               wire:click="destroy({{$notification->id}})"
    ...
    <div class="intro-y col-span-12 flex flex-wrap sm:flex-row sm:flex-nowrap items-center">
        ...
        <div class="form-inline ml-auto">
            {{ $notifications->links('admin.layouts.livewire_pagination') }}
        </div>
